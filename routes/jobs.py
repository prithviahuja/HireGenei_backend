...
async def scrape_jobs(request: JobScrapeRequest):
    """Non-streaming: returns all jobs at once (kept for compatibility/fallback)."""
    ...
    try:
        ...
        logger.info(f"Returning {len(jobs)} jobs (non-streaming).")
...
async def scrape_jobs_stream_route(request: JobScrapeRequest):
    """Streaming (NDJSON): emits each job the moment it's found so the UI can
    show results live instead of waiting for the whole scrape to finish."""
    ...
    async def generate():
        ...
        try:
            ...
            if count == 0:
                yield json.dumps({
                    "type": "warning",
                    "detail": "No jobs found. Try widening your filters or try again in a moment — LinkedIn may be rate-limiting.",
                }) + "\n"
